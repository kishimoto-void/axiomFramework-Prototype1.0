...
impl fmt::Display for PssError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            PssError::DeserializationFailed(msg) => {
                write!(f, "[PSS Deserialization Error] {}", msg)
            }
        ...
        Phase::Clarify => {
            ...
            if !pending.is_empty() {
                ...
                let pending_names: Vec<String> = pending
                    ...
                    .collect();
                blocking.push(format!("必須の不足情報質問が未完了: {:?}", pending_names));
            }
...
impl ProblemSpecification {
    ...
    pub fn to_json(&self) -> Result<String, PssError> {
        serde_json::to_string_pretty(self).map_err(|e| PssError::SerializationFailed(e.to_string()))
    }

    pub fn from_json(json: &str) -> Result<Self, PssError> {
        serde_json::from_str(json).map_err(|e| PssError::DeserializationFailed(e.to_string()))
    }
...
impl ProblemBuilder {
    ...
    pub fn phase(mut self, phase: Phase, cycle: u32, scope: &str, scope_agreed: bool) -> Self {
...
pub fn compile_for_generic(spec: &ProblemSpecification, mode: &str) -> String {
    ...
    let pred = if pq.is_prediction_allowed {
        ...
        match spec.prediction_policy.when_uncertain {
            ...
            BehaviorAction::StateUnknown => {
                "根拠が足りないので、「現時点では判断できない」と明示する。".to_string()
            }
            action => format!(
                "根拠が足りないので、方針「{}」に従う（断定しない）。",
                action.as_str()
            ),
        }
    };
    ...
    if let Some(ref g) = gate {
        if !g.can_proceed {
            ...
            match spec.phase_state.phase {
                Phase::Clarify => core_lines
                    .push("【進行】整理・確認フェーズ。不明点があれば先に聞く。".to_string()),
                Phase::Confirm => {
                    core_lines.push("【進行】確認済みの範囲で答えてよい。".to_string())
                }
                Phase::Answer => core_lines
                    .push("【進行】回答フェーズ。仕様の範囲で結論を出してよい。".to_string()),
            }
        }
    }
...
mod tests {
    ...
    fn test_validation_rules() {
        ...
        assert!(report
            .findings
            .iter()
            .any(|f| f.code == "INVALID_ID_FORMAT"));
    }
}
