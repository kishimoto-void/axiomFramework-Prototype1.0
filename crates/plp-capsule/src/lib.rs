    ...
    for ch in s.chars() {
        match ch {
            ...
            _ => {
                ...
                out.write_all(encoded.as_bytes()).map_err(CapsuleError::io)?;
            }
        }
    }
    out.write_all(b"\"").map_err(CapsuleError::io)
...
fn write_canonical_f64<W: Write + ?Sized>(out: &mut W, val: f64) -> Result<(), CapsuleError> {
    ...
    if let Some(pos) = s.find("e+") {
        out.write_all(s[..pos + 1].as_bytes()).map_err(CapsuleError::io)?;
        out.write_all(s[pos + 2..].as_bytes()).map_err(CapsuleError::io)?;
    } else {
...
pub fn compute_content_hash<H: HashAlgorithm>(
    ...
    hasher.compute_streaming(|writer| {
        writer.write_all(b"{\"header\":{\"protocol\":")
            .map_err(CapsuleError::io)?;
        write_raw_str(writer, &header.protocol)?;
        writer.write_all(b",\"capsule_schema\":")
            .map_err(CapsuleError::io)?;
        write_raw_str(writer, &header.capsule_schema)?;
        writer.write_all(b",\"version\":").map_err(CapsuleError::io)?;
        write_raw_str(writer, &header.version)?;
        writer.write_all(b",\"capsule_id\":")
            .map_err(CapsuleError::io)?;
        write_raw_str(writer, &header.capsule_id)?;
        writer.write_all(b",\"parent_id\":")
            .map_err(CapsuleError::io)?;
        ...
        writer.write_all(b",\"sequence\":").map_err(CapsuleError::io)?;
        write_i64(writer, header.sequence)?;
        writer.write_all(b",\"timestamp_ns\":\"")
            .map_err(CapsuleError::io)?;
        write_u128(writer, header.timestamp_ns)?;
        writer.write_all(b"\",\"source\":").map_err(CapsuleError::io)?;
        write_raw_str(writer, &header.source)?;
        writer.write_all(b",\"is_keyframe\":")
            .map_err(CapsuleError::io)?;
        writer.write_all(if header.flags.is_keyframe { b"true" } else { b"false" })
            .map_err(CapsuleError::io)?;
        writer.write_all(b",\"hash_algorithm\":")
            .map_err(CapsuleError::io)?;
        write_raw_str(writer, &header.hash_algorithm)?;

        writer.write_all(b"},\"observations\":[")
            .map_err(CapsuleError::io)?;
        ...
        writer.write_all(b"],\"delta\":{").map_err(CapsuleError::io)?;
        for (i, (key, entry)) in delta.changes.iter().enumerate() {
            ...
            writer.write_all(b",\"values\":{").map_err(CapsuleError::io)?;
...
fn write_observation_block<W: Write + ?Sized>(
    ...
    writer.write_all(b",\"schema\":").map_err(CapsuleError::io)?;
    write_raw_str(writer, &o.schema)?;
    writer.write_all(b",\"capability\":").map_err(CapsuleError::io)?;
    write_raw_str(writer, &o.capability)?;
    writer.write_all(b",\"observer_id\":").map_err(CapsuleError::io)?;
...
impl PLPCapsule {
    pub fn verify<H: HashAlgorithm>(&self, hasher: &H) -> Result<bool, CapsuleError> {
        let calculated = compute_content_hash(&self.header, &self.observations, &self.delta, hasher)?;
