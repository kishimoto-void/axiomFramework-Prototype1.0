...
impl RuntimePolicy {
    pub fn evaluate(
        ...
        if *primitive == ReasoningPrimitive::Commit && self.require_validation_before_commit {
            let has_validation = session
                .transitions
                .iter()
                .rev()
                .any(|t| t.primitive == ReasoningPrimitive::Validation && t.validation_passed);
...
impl StateHasher {
    pub fn compute_hash(state: &ReasoningState) -> Result<CapsuleHash, LrpError> {
        let value = serde_json::to_value(state)
            .map_err(|e| LrpError::SerializationError(format!("State to_value failed: {}", e)))?;
    ...
    pub fn validate(
        ...
        for v in &self.validators {
            let res = catch_unwind(AssertUnwindSafe(|| {
                v.validate_transition(session, primitive)
            }));
            match res {
                Ok(Ok(())) => {}
                Ok(Err(e)) => {
            ...
            match (&delta.action, &delta.kind, &delta.payload) {
                ...
                (DeltaAction::Added, DeltaKind::EvidenceChange, TypedPayload::Evidence(ev)) => {
                    if !next
                        .evidence
                        .iter()
                        .any(|e| e.evidence_id == ev.evidence_id)
                    {
...
impl LRPKernelRuntime {
    ...
    pub fn create_session(
        ...
        let mut session = ReasoningSession {
            ...
            seed: self.seed,
            snapshot_interval: if snapshot_interval == 0 {
                100
            } else {
                snapshot_interval
            },
    ...
    fn execute_transition(
        ...
        if session
            .transitions
            .len()
            .is_multiple_of(session.snapshot_interval)
        {
