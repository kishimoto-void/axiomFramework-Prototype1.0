...
//! Standard Protocol Implementation (RFC/Production Edition)
//!
//! Refinements in this edition:
//! 1. IEEE 754 Bit-exact Canonical Hash (StateHasher)
//! 2. High-performance Relaxed Clock Ordering
//! 3. Reusable TransitionBuilder (&self execution)
//! 4. ReasoningIntent Primitive Integration
//! 5. EventSeverity for Production Audit Logs
//! 6. Protocol-version-aware Merkle Runtime Hash
//! 7. Flexible Snapshot Modes (Full, Delta, Compressed)
//! 8. Plugin Execution Policy Framework
...
/// AXIOM Framework 拡張: 遷移の「意図」を明示的に表現・監査ログ化
#[derive(Debug, Clone, PartialEq, Eq, Hash, Serialize, Deserialize, PartialOrd, Ord)]
...
impl StateHasher {
    /// Canonical State Hashing (IEEE 754 Bit-exact Determinism)
    /// - 64文字フル長さのSHA-256
    /// - JSONオブジェクトのキーソート保証
    /// - 浮動小数点 (f64) は IEEE 754 ビットパターン (16進表現) に直接正規化して決定性を保証
        ...
        match value {
            ...
            serde_json::Value::Number(num) => {
                if let Some(f) = num.as_f64() {
                    // IEEE 754 ビット列の厳密な文字列化により決定性を完全保証
...
impl ReasoningTransition {
    /// デリミタ "|" および プロトコル VERSION を含めたフル長SHA-256 Merkle Chain Hash
...
impl TransitionBuilder {
    ...
    /// &self 実行により、Builder テンプレートの再利用を可能に改善
    pub fn execute(
        &self,
...
impl ReasoningSession {
    ...
    /// 改ざん検知・再現性保証のための完全ハッシュチェーン＆スナップショット状態検証
...
// =============================================================================
// 14. High-performance Lock-free Clock (Relaxed Ordering)
// =============================================================================
...
impl LRPKernelRuntime {
    ...
    fn execute_transition(
        ...
        if session.transitions.len() % session.snapshot_interval == 0 {
...
mod tests {
    ...
    fn test_ieee754_bit_exact_hash_determinism() {
        ...
        state_b.candidates[0].score = 0.8500000001; // 完全同一値
        ...
        assert_eq!(
            hash_a, hash_b,
            "Exact IEEE 754 float values must produce identical hashes"
        );
    ...
    fn test_builder_reusability_and_reasoning_intent() {
        ...
        // &self 実行により同じ Builder テンプレートを連続利用可能
        template_builder.execute(&runtime, &mut session).unwrap();
        template_builder.execute(&runtime, &mut session).unwrap();

        assert_eq!(session.transitions.len(), 2);
        assert_eq!(
            session.transitions[0].intent,
            ReasoningIntent::Exploration
        );
        assert_eq!(
            session.transitions[1].intent,
            ReasoningIntent::Exploration
        );
    }

    #[test]
    fn test_chain_verification_with_version() {
        let runtime = LRPKernelRuntime::new(300, vec![], vec![]);
        let mut session = runtime
            .create_session("version_hash_test", Vector::new(), Vector::new(), None, 1)
            .unwrap();

        runtime
            .builder(ReasoningPrimitive::Plan, "Init")
