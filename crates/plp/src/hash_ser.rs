...
pub fn build_canonical_payload(header: &ProjectionHeader, canonical: &CanonicalState) -> Vec<u8> {
    ...
    parts.push(format!(
        "{}:{}",
        write_raw_str("protocol"),
        write_raw_str(&header.protocol)
    ));
    parts.push(format!(
        ",{}:{}",
        write_raw_str("version"),
        write_raw_str(&header.version)
    ));
    parts.push(format!(
        ",{}:{}",
        write_raw_str("capsule_id"),
        write_raw_str(&header.capsule_id)
    ));
    ...
    parts.push(format!(
        ",{}:{}",
        write_raw_str("source"),
        write_raw_str(&header.source)
    ));
    parts.push(format!(
        ",{}:{}",
    ...
    parts.push(format!(
        "{}:{}",
        write_raw_str("version"),
        write_raw_str(&canonical.version)
    ));
    parts.push(format!(
        ",{}:{}",
        write_raw_str("language"),
        write_raw_str(&canonical.language)
    ));
    ...
    for (i, a) in anns.iter().enumerate() {
        ...
        parts.push(format!(
            "{}:{}",
            write_raw_str("kind"),
            write_raw_str(&a.kind)
        ));
        parts.push(format!(
            ",{}:{}",
            write_raw_str("value"),
            write_raw_str(&a.value)
        ));
